...
from pyspark.sql.types import StructField, StructType, StringType, BooleanType, ArrayType, DateType, IntegerType, FloatType, TimestampType
...
bankDepositRawStreamDF = spark.readStream \
    .format("kafka") \
...
bankDepositStreamDF = bankDepositRawStreamDF.selectExpr("cast(key as string) as key", "cast(value as string) as value")
#TO-DO: using the kafka message StructType, deserialize the JSON from the streaming dataframe 
bankDepositStreamDF.withColumn("value", from_json("value", bankDepositSchema)) \
    .select(col("value.*")) \
...
bankDepositStarStreamDF = spark.sql("select * from BankDeposits")
...
bankDepositStarStreamDF.writeStream \
    .format("console") \
    .outputMode("append") \
    .start() \
    .awaitTermination() 
