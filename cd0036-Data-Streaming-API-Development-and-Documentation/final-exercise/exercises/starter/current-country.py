...
redisServerStreamingRawDF = spark.readStream \
.format("kafka") \
...
redisServerStarStreamingDF = redisServerStreamingRawDF.selectExpr("cast(key as string) as key", "cast(value as string) as value")
#TO-DO: using the redisMessageSchema StructType, deserialize the JSON from the streaming dataframe 
redisServerStarStreamingDF.withColumn("value", from_json("value", redisMessageSchema)) \
    .select(col("value.*")) \
...
RedisDataStreamingDF = spark.sql("select key, zSetEntries[0].element as redisEvent from RedisData")
...
zSetDecodedEntriesStreamingDF1= RedisDataStreamingDF.withColumn("redisEvent", unbase64(RedisDataStreamingDF.redisEvent).cast("string"))

#TO-DO: repeat this a second time, so now you have two separate dataframes that contain redisEvent data
zSetDecodedEntriesStreamingDF2= RedisDataStreamingDF.withColumn("redisEvent", unbase64(RedisDataStreamingDF.redisEvent).cast("string"))
...
zSetDecodedEntriesStreamingDF1.withColumn(
    "redisEvent", from_json("redisEvent", customerSchema)
).select(col("redisEvent.*")).createOrReplaceTempView("Customer")
...
zSetDecodedEntriesStreamingDF2.withColumn(
    "redisEvent", from_json("redisEvent", customerLocationSchema)
).select(col("redisEvent.*")).createOrReplaceTempView("CustomerLocation")
...
customerDF1 = spark.sql("select accountNumber as customerAccountNumber, location as homeLocation, birthDay from Customer where birthDay is not null")
#TO-DO: select the customerAccountNumber, homeLocation, and birth year (using split)
relevantCustomerFieldsStreamingDF = customerDF1.select(
...
customerLocationDF1 = spark.sql("select accountNumber as locationAccountNumber, location from CustomerLocation")
#TO-DO: join the customer and customer location data using the expression: customerAccountNumber = locationAccountNumber
currentAndHomeLocationStreamingDF = customerLocationDF1.join(relevantCustomerFieldsStreamingDF, expr(
    """
    locationAccountNumber = customerAccountNumber
    """
))
...
# +---------------------+-----------+---------------------+------------+---------+
currentAndHomeLocationStreamingDF.writeStream \
.format("console") \
