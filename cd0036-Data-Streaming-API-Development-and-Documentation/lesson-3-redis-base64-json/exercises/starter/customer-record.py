...
readServerRawStreamDF = spark.readStream \
    .format("kafka") \
...
readServerStreamDF = readServerRawStreamDF.selectExpr(
    "cast(key as string) as key",
    "cast(value as string) as value"
)
#TO-DO: using the redisMessageSchema StructType, deserialize the JSON from the streaming dataframe 
readServerStreamDF.withColumn("value", from_json("value", redisMessageSchema)) \
    .select(col("value.*")) \
...
zSetEntriesStreamingDF = zSetEntriesEncodedStreamingDF.withColumn(
    "customer", unbase64(zSetEntriesEncodedStreamingDF.customer).cast("string"))
...
zSetEntriesStreamingDF.withColumn("value", from_json("customer", customerSchema)) \
    .select(col("value.*")) \
