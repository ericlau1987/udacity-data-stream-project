...
class Line:
    ...
    def _handle_arrival(self, message):
        ...
        if prev_dir is not None and prev_station_id is not None:
            ...
            logger.debug(f"prev_station: {prev_station}")
    ...
    def process_message(self, message):
        ...
        logger.debug(f"{message.topic()}: {message.value()}")
        if 'org.chicago.cta.stations.table' == message.topic(): # Set the conditional correctly to the stations Faust Table
